...
impl QdrantVectorDb {
    pub fn new(url: &str) -> Result<Self> {
        ...
        Ok(Self { client: Arc::new(client), rest_base })
    }
...
impl VectorDatabase for QdrantVectorDb {
            ...
            match self.client.collection_exists(name).await {
                Ok(exists) => {
                    if !exists {
                        ...
                        if let Err(e) = self.client.create_collection(req).await {
                            ...
                            if !resp.status().is_success() { anyhow::bail!("REST create_collection HTTP {}", resp.status()); }
                        }
                    }
                    Ok(())
                }
                Err(e) => {
                    ...
                    if resp.status() == reqwest::StatusCode::NOT_FOUND {
                        ...
                        if !resp2.status().is_success() { anyhow::bail!("REST create_collection HTTP {}", resp2.status()); }
                    }
                    Ok(())
                }
            }
    ...
    fn drop_collection(&self, name: &str) -> Result<()> {
        run_async_blocking(async {
            if !self.client.collection_exists(name).await.unwrap_or(false) { return Ok(()); }
            match self.client.delete_collection(name).await {
                Ok(_) => Ok(()),
                Err(e) => {
                    eprintln!("[warn] gRPC delete_collection failed: {} — falling back to REST", e);
                    let url = format!("{}/collections/{}", self.rest_base.trim_end_matches('/'), name);
                    let client = reqwest::Client::new();
                    let resp = client.delete(url).send().await?;
                    if !resp.status().is_success() { anyhow::bail!("REST delete_collection HTTP {}", resp.status()); }
                    Ok(())
                }
            }
        })
    ...
    fn insert(&self, name: &str, points: &[VectorInsert]) -> Result<()> {
        ...
        let pts: Vec<PointStruct> = points.iter().map(|p| {
    ...
    fn search(&self, name: &str, vector: &[f32], top_k: usize) -> Result<Vec<VectorHit>> {
        run_async_blocking(async move {
            ...
            let result = match self.client.query(req).await {
                Ok(r) => r,
                Err(e) => {
                    eprintln!("[warn] gRPC query failed: {} — falling back to REST /points/search", e);
            ...
            let out = result.result.into_iter().filter_map(|p| {
                ...
                let int_of = |k: &str| -> i64 {
                    payload.get(k).and_then(|v| match v.kind { Some(Vk::IntegerValue(n)) => Some(n), _ => None }).unwrap_or(0)
                };
                Some(VectorHit {
    ...
    fn delete_by_relative_paths(&self, name: &str, rel_paths: &[String]) -> Result<()> {
        ...
        run_async_blocking(async move {
            ...
            match self.client.delete_points(req).await {
                Ok(_) => Ok(()),
                Err(e) => {
                    eprintln!("[warn] gRPC delete failed: {} — falling back to REST /points/delete", e);
