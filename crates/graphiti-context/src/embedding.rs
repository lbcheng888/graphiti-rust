...
pub mod qwen3_embed_anything {
    ...
    pub struct Qwen3EmbedAnything {
        embedder: Arc<Embedder>,
        dim: usize,
    }
    impl Qwen3EmbedAnything {
    ...
    impl EmbeddingModel for Qwen3EmbedAnything {
        fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
            ...
            let out: Vec<EmbedData> = rt
                .block_on(async { embed_query(&refs, &self.embedder, None).await })
        ...
        fn name(&self) -> &'static str {
            "qwen3-embedding-0.6b-embed-anything"
        }
    }
}
...
pub mod gemma_candle {
    ...
    pub struct CandleGemmaEmbedding {
        dim: usize,
        device: Device,
        emb_weight: Tensor,
    ...
    impl CandleGemmaEmbedding {
        pub fn load(model_dir: Option<PathBuf>) -> Result<Self> {
            ...
            let (tokenizer_opt, hidden_size, vocab_size) = load_tokenizer_and_config(&model_dir)
                .unwrap_or_else(|e| {
                    eprintln!("[gemma] tokenizer/config load failed: {e}");
                    let cfg_path = model_dir.join("config.json");
                    let cfg_text = fs::read_to_string(&cfg_path).unwrap_or_else(|_| "{}".into());
                    let cfg_json: JsonValue =
                        serde_json::from_str(&cfg_text).unwrap_or(JsonValue::Null);
                    let hidden = pick_usize(
                        &cfg_json,
                        &["hidden_size", "n_embed", "model_dim", "d_model"],
                    )
                    .unwrap_or(Some(768))
                    .unwrap_or(768);
                    let vocab = pick_usize(&cfg_json, &["vocab_size"])
            ...
            let mut candidates: Vec<String> = vec![
                ...
                "model.vocab_emb.weight",
                "model.text_model.embeddings.word_embeddings.weight",
                "transformer.wte.weight",
                "shared.weight",
                "model.encoder.embed_tokens.weight",
                "model.decoder.embed_tokens.weight",
                "model.embedder.weight",
                ...
                if let Ok(text) = std::fs::read_to_string(&idx_path) {
                    if let Ok(j) = serde_json::from_str::<serde_json::Value>(&text) {
                        if let Some(map) = j.get("weight_map").and_then(|v| v.as_object()) {
                            ...
                            for k in map.keys() {
                                if candidates.iter().any(|c| k == c) {
                                    present.push(k.clone());
                                }
                            }
                            for k in map.keys() {
                                let ks = k.to_lowercase();
                                if (ks.contains("embed") || ks.contains("token"))
                                    && ks.ends_with(".weight")
                                {
            ...
            for name in &candidates {
                if let Ok(w) = vb_get_tensor(&vb, name.as_str(), (vocab_size, hidden_size), &device)
                {
                    emb_weight = Some(w);
                    break;
                }
            ...
            if emb_weight.is_none() {
                ...
                if path.exists() {
                    if let Ok(t) =
                        super::load_safetensors_matrix(&path, &candidates, vocab_size, hidden_size)
                    {
                        emb_weight = Some(Tensor::from_vec(t, (vocab_size, hidden_size), &device)?);
                    }
                }
            }
            let emb_weight = emb_weight
                .ok_or_else(|| anyhow!("embedding weight not found in model.safetensors"))?;
            Ok(Self {
                dim: hidden_size,
                device,
                emb_weight,
    ...
    impl EmbeddingModel for CandleGemmaEmbedding {
        fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
            let mut out = Vec::with_capacity(texts.len());
            for t in texts {
                let ids: Vec<u32> = if let Some(tok) = &self.tokenizer {
                    let e = tok
                        .encode(t.as_str(), true)
                        .map_err(|e| anyhow!("tokenize: {e}"))?;
                    e.get_ids().iter().map(|&i| i as u32).collect()
                } else {
                    let mut v = Vec::new();
                    for w in t.split_whitespace() {
                        ...
                        v.push((h as usize % self.vocab_size) as u32);
                    }
                    v
                };
                if ids.is_empty() {
                    out.push(vec![0f32; self.dim]);
                    continue;
                }
    ...
    fn resolve_model_dir(model_dir: Option<PathBuf>) -> Result<(PathBuf, bool)> {
        ...
        if let Ok(p) = std::env::var("EMBEDDING_MODEL_DIR") {
            return Ok((PathBuf::from(p), false));
        }
        let api = Api::new()?;
        let model = api.model("google/embeddinggemma-300m".to_string());
        let _ = model.get("model.safetensors.index.json");
        let _ = model.get("model.safetensors");
        let _ = model.get("config.json");
        let tok = model.get("tokenizer.json")?;
        let dir = tok
            .parent()
            .ok_or_else(|| anyhow!("cannot get model dir from tokenizer path"))?
            .to_path_buf();
        Ok((dir, true))
    }
    fn load_tokenizer_and_config(model_dir: &Path) -> Result<(Option<Tokenizer>, usize, usize)> {
        ...
        let cfg_json: JsonValue = serde_json::from_str(&cfg_text)?;
        let hidden_size = pick_usize(
            &cfg_json,
            &["hidden_size", "n_embed", "model_dim", "d_model"],
        )?
        .ok_or_else(|| anyhow!("hidden size not found in config"))?;
        let vocab_size = pick_usize(&cfg_json, &["vocab_size"])?
            .ok_or_else(|| anyhow!("vocab size not found in config"))?;
        Ok((Some(tokenizer), hidden_size, vocab_size))
    }
    fn pick_usize(v: &JsonValue, keys: &[&str]) -> Result<Option<usize>> {
        for k in keys {
    ...
    fn mmap_var_builder(model_dir: &Path) -> Result<VB> {
        let device = Device::Cpu;
        let idx = model_dir.join("model.safetensors.index.json");
        if idx.exists() {
            let idx_text = fs::read_to_string(&idx)?;
            let j: JsonValue = serde_json::from_str(&idx_text)?;
            let mut shards = std::collections::BTreeSet::new();
            if let Some(wm) = j.get("weight_map").and_then(|x| x.as_object()) {
                for (_, file) in wm.iter() {
                    if let Some(f) = file.as_str() {
                        shards.insert(f.to_string());
                    }
                }
            }
            let paths: Vec<PathBuf> = shards.into_iter().map(|f| model_dir.join(f)).collect();
            let vb = unsafe {
                candle_nn::VarBuilder::from_mmaped_safetensors(&paths, DType::F32, &device)?
            };
            Ok(VB(vb))
        } else {
            let path = model_dir.join("model.safetensors");
    ...
    fn vb_get_tensor(
        ...
        let (vocab, dim) = shape;
        if let Ok(t) = vb.0.get((vocab, dim), name) {
            return Ok(t);
        }
        if let Ok(t) = vb.0.get((dim, vocab), name) {
            return Ok(t.transpose(0, 1)?);
        }
        Err(anyhow!(
            "tensor not found or shape mismatch for key: {}",
            name
        ))
    }
}
...
#[cfg(feature = "candle-gemma")]
pub fn load_safetensors_matrix(
    path: &std::path::Path,
    candidates: &[String],
    vocab: usize,
    dim: usize,
    ...
    for name in candidates {
        if let Ok(t) = st.tensor(&name) {
            let shape = t.shape();
            if shape == [vocab, dim] || shape == [dim, vocab] {
                let mut buf = vec![0f32; (shape[0] * shape[1]) as usize];
                t.data()
                    .read_exact(bytemuck::cast_slice_mut(&mut buf))
                    .map_err(|e| anyhow::anyhow!("read tensor data: {e}"))?;
                if shape == [dim, vocab] {
                    let mut tr = vec![0f32; buf.len()];
                    for i in 0..dim {
                        for j in 0..vocab {
                            tr[j * dim + i] = buf[i * vocab + j];
                        }
                    }
                    return Ok(tr);
                }
