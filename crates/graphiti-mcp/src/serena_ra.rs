...
pub async fn list_roots() -> Vec<PathBuf> {
    ...
    if let Some(map) = reg.as_ref() { map.keys().cloned().collect() } else { vec![] }
}
...
pub async fn status(root: Option<&Path>) -> serde_json::Value {
    ...
    None => {
        ...
        let roots: Vec<String> = roots.into_iter().map(|p| p.to_string_lossy().to_string()).collect();
...
fn resolve_ra_binary() -> String {
    ...
    if let Ok(p) = std::env::var("RUST_ANALYZER_BIN") { return p; }
    if let Ok(p) = std::env::var("RA_BINARY") { return p; }
...
pub async fn get_or_start(root: &Path) -> Result<Arc<RaClient>> {
    {
        ...
        if let Some(map) = reg.as_ref() {
            if let Some(c) = map.get(root) { return Ok(c.clone()); }
        }
    }
    ...
    let client = Arc::new(RaClient { root: root.to_path_buf(), stdin: Arc::new(Mutex::new(stdin)), pending: Arc::new(Mutex::new(HashMap::new())), id: Arc::new(AtomicI64::new(1)) });
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let n = match content_length { Some(v) => v, None => return };
            ...
            if let Some(id) = val.get("id").and_then(|v| v.as_i64()) {
                ...
                if let Some(tx) = p.remove(&id) { let _ = tx.send(val); }
            }
        }
    });

    // initialize
    let root_uri = url::Url::from_file_path(root).map_err(|_| anyhow!("invalid root path"))?.to_string();
    let params = json!({
        "processId": null,
        "rootUri": root_uri,
        "capabilities": {},
    ...
    client.notify("initialized", json!({"capabilities":{}})).await?;
...
impl RaClient {
    ...
    pub async fn request(&self, method: &str, params: serde_json::Value) -> Result<serde_json::Value> {
        ...
        self.send_raw(&req).await?;
        let secs = std::env::var("GRAPHITI_RA_REQUEST_TIMEOUT_SECS").ok().and_then(|v| v.parse::<u64>().ok()).unwrap_or(10);
        ...
        if let Some(err) = resp.get("error") { return Err(anyhow!("lsp error: {}", err)); }
        Ok(resp["result"].clone())
    }
}

fn uri_for(path: &Path) -> Result<String> { Ok(url::Url::from_file_path(path).map_err(|_| anyhow!("bad path"))?.to_string()) }
...
pub async fn workspace_symbol(root: &Path, query: &str) -> Result<serde_json::Value> {
    let client = get_or_start(root).await.map_err(|e| anyhow!("rust-analyzer unavailable: {}", e))?;
    client.request("workspace/symbol", json!({"query": query})).await
}

pub async fn references(root: &Path, file: &Path, line: u32, character: u32) -> Result<serde_json::Value> {
    let client = did_open(root, file).await.map_err(|e| anyhow!("rust-analyzer unavailable: {}", e))?;
    let uri = uri_for(file)?;
...
fn pos_in_range(line:u32, character:u32, range:&serde_json::Value)->bool{
    let s = &range["start"]; let e=&range["end"];
    let sl=s["line"].as_u64().unwrap_or(0) as u32; let sc=s["character"].as_u64().unwrap_or(0) as u32;
    let el=e["line"].as_u64().unwrap_or(0) as u32; let ec=e["character"].as_u64().unwrap_or(0) as u32;
...
pub async fn symbol_at_position(root:&Path, file:&Path, line:u32, character:u32)->Result<Option<(String,u64,serde_json::Value)>>{
    let doc = document_symbols(root, file).await?;
    fn walk<'a>(arr:&'a serde_json::Value, line:u32, character:u32, out:&mut Option<(&'a serde_json::Value, &'a serde_json::Value, u64)>){
        if let Some(items)=arr.as_array(){
            for it in items{
                let range=&it["range"]; if pos_in_range(line,character,range){
                    let kind=it["kind"].as_u64().unwrap_or(0);
                    *out = Some((&it["name"], range, kind));
                    if let Some(children)=it.get("children"){ walk(children,line,character,out); }
                }
            }
        }
    }
    let mut sel=None; walk(&doc,line,character,&mut sel);
    if let Some((name_v, range, kind))=sel { Ok(Some((name_v.as_str().unwrap_or("").to_string(), kind, range.clone()))) } else { Ok(None) }
}

pub async fn definition(root:&Path, file:&Path, line:u32, character:u32)->Result<serde_json::Value>{
    let client = did_open(root,file).await.map_err(|e| anyhow!("rust-analyzer unavailable: {}", e))?;
    let uri = uri_for(file)?;
...
pub async fn find_target_symbol_name(root:&Path, def_uri:&str, def_range:&serde_json::Value)->Option<String>{
        ...
        if let Ok(path)=url.to_file_path(){
            if let Ok(doc)=document_symbols(root, &path).await{
                ...
                fn covers(pos_l:u32, pos_c:u32, obj:&serde_json::Value)->bool{
                    if let Some(sr)=obj.get("selectionRange"){ return super::serena_ra::pos_in_range(pos_l,pos_c,sr); }
                    super::serena_ra::pos_in_range(pos_l,pos_c,&obj["range"]) 
                }
                fn walk<'a>(arr:&'a serde_json::Value, pos_l:u32, pos_c:u32)->Option<&'a serde_json::Value>{
                    if let Some(a)=arr.as_array(){
                        // prefer最深层覆盖 selectionRange/range 的符号
                        for it in a {
                            let has = covers(pos_l,pos_c,it);
                            if let Some(ch)=it.get("children"){ if let Some(n)=walk(ch,pos_l,pos_c){ return Some(n); } }
                            if has { return Some(&it["name"]); }
                        }
                    }
                    None
                }
                if let Some(n)=walk(&doc, dl, dc){ return n.as_str().map(|s| s.to_string()); }
            }
        }
    }
    None
