...
impl LocalIndexStorage {
    ...
    pub fn collection_dir(&self, name: &str) -> Result<PathBuf> {
    ...
    pub fn has_collection(&self, name: &str) -> Result<bool> {
        Ok(self.base_dir.join(name).is_dir())
    }
    ...
    pub fn insert_chunks(&self, name: &str, chunks: &[CodeChunk]) -> Result<()> {
        let c = self.ensure_collection(name)?;
    ...
    pub fn search(&self, name: &str, query: &str, top_k: usize) -> Result<Vec<SearchResult>> {
        ...
        for line in reader.lines() {
            ...
            let lc = chunk.content.to_ascii_lowercase();
            let mut score = 0f32;
    ...
    pub fn remove_file(&self, name: &str, relative_path: &str) -> Result<()> {
        ...
        while reader.read_line(&mut buf)? > 0 {
            ...
            if !line.is_empty() {
                if let Ok(ch) = serde_json::from_str::<CodeChunk>(line) {
                    if ch.relative_path != relative_path {
