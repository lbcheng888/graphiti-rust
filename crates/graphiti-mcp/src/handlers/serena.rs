...
use crate::types::{AppState, AddCodeEntityRequest, AddMemoryRequest};
use uuid::Uuid;
use graphiti_core::storage::{GraphStorage, Direction};
use graphiti_core::graph::{EntityNode, Edge, TemporalMetadata};
...
fn parallelism(default: usize) -> usize {
    if let Ok(s) = std::env::var("GRAPHITI_RA_CONCURRENCY") {
        if let Ok(n) = s.parse::<usize>() { return n.clamp(1, 128); }
    }
    default
}

async fn edge_exists(state: &AppState, source: Uuid, target: Uuid, relation: &str) -> Result<bool, StatusCode> {
    let edges = state
        .storage
        .get_edges(&source, Direction::Outgoing)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(edges.iter().any(|e| e.target_id == target && e.relationship == relation))
}
...
#[derive(Deserialize)]
pub struct GetConfigArgs { pub project: Option<String> }

pub async fn get_current_config(args: GetConfigArgs) -> Result<serde_json::Value, StatusCode> {
    let project = args.project.as_deref();
    let root = crate::serena_config::detect_project_root(project.map(Path::new))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn replace_lines_and_record(args: ReplaceLinesAndRecordArgs) -> Result<&'static str, StatusCode> {
    // 1) 应用文件修改
    crate::serena_tools::replace_lines(
        crate::serena_tools::ReplaceLinesRequest{
...
#[derive(Deserialize)]
pub struct SymbolsOverviewArgs { pub path: String }

pub async fn get_symbols_overview(args: SymbolsOverviewArgs) -> Result<serde_json::Value, StatusCode> {
...
#[derive(Deserialize)]
pub struct FindSymbolArgs { pub root: String, pub name: String }

pub async fn find_symbol(args: FindSymbolArgs) -> Result<serde_json::Value, StatusCode> {
    let hits = crate::serena_symbols::find_symbol(&crate::serena_symbols::FindSymbolRequest{ root: args.root.into(), name: args.name })
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
#[derive(Deserialize)]
pub struct RaDocumentSymbolsArgs { pub root: String, pub path: String }
#[derive(Deserialize)]
pub struct RaWorkspaceSymbolArgs { pub root: String, pub query: String }
#[derive(Deserialize)]
pub struct RaReferencesArgs { pub root: String, pub path: String, pub line: usize, pub character: usize }
#[derive(Deserialize)]
pub struct RaDefinitionArgs { pub root: String, pub path: String, pub line: usize, pub character: usize }
#[derive(Deserialize)]
pub struct RaRootArgs { pub root: String }

pub async fn ra_document_symbols(args: RaDocumentSymbolsArgs) -> Result<serde_json::Value, StatusCode> {
...
pub async fn ra_workspace_symbol(args: RaWorkspaceSymbolArgs) -> Result<serde_json::Value, StatusCode> {
...
pub async fn ra_references(args: RaReferencesArgs) -> Result<serde_json::Value, StatusCode> {
    crate::serena_ra::references(Path::new(&args.root), Path::new(&args.path), args.line as u32, args.character as u32)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
pub async fn ra_definition(args: RaDefinitionArgs) -> Result<serde_json::Value, StatusCode> {
    crate::serena_ra::definition(Path::new(&args.root), Path::new(&args.path), args.line as u32, args.character as u32)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
#[derive(Deserialize)]
pub struct RecentEdgesArgs { pub relationship: String, pub limit: Option<usize> }
#[derive(Deserialize)]
pub struct PruneEdgesArgs { pub limit: usize }

pub async fn graph_summary(state: &AppState) -> Result<serde_json::Value, StatusCode> {
    let nodes = state.storage.get_all_nodes().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let edges = state.storage.get_all_edges().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn recent_edges(state: &AppState, args: RecentEdgesArgs) -> Result<serde_json::Value, StatusCode> {
    let list = state
        .storage
...
#[derive(Deserialize)]
pub struct FindRefsArgs { pub root: String, pub name: String }

pub async fn find_referencing_symbols(args: FindRefsArgs) -> Result<serde_json::Value, StatusCode> {
    let hits = crate::serena_symbols::find_references(&crate::serena_symbols::FindRefsRequest{ root: args.root.into(), name: args.name })
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(serde_json::to_value(hits).unwrap_or(serde_json::json!([])))
}

#[derive(Deserialize)]
pub struct ReplaceSymbolBodyArgs { pub path: String, pub name: String, pub new_body: String }

pub async fn replace_symbol_body(args: ReplaceSymbolBodyArgs) -> Result<&'static str, StatusCode> {
    crate::serena_symbols::replace_symbol_body(&crate::serena_symbols::ReplaceSymbolBodyRequest{ path: args.path.into(), name: args.name, new_body: args.new_body })
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok("ok")
}
...
#[derive(Deserialize)]
pub struct RaDocSymbolsToEntitiesArgs { pub root: String, pub path: String, pub limit: Option<usize> }

pub async fn ra_document_symbols_to_entities(state: &AppState, args: RaDocSymbolsToEntitiesArgs) -> Result<String, StatusCode> {
    let res = ra_document_symbols(RaDocumentSymbolsArgs{ root: args.root.clone(), path: args.path.clone() }).await?;
    fn collect(v:&serde_json::Value,out:&mut Vec<(String,(u32,u32))>){
        if let Some(arr)=v.as_array(){ for it in arr { collect(it,out);} return; }
        if let Some(o)=v.as_object(){
            let name=o.get("name").and_then(|s|s.as_str()).unwrap_or("").to_string();
            let r=o.get("range");
            ...
            if !name.is_empty(){ out.push((name,(s,e))); }
            if let Some(ch)=o.get("children"){ collect(ch,out); }
        }
    }
    let mut syms=Vec::new(); collect(&res,&mut syms);
    if let Some(l)=args.limit{ syms.truncate(l); }
...
pub struct RaBuildWorkspaceArgs {
    pub root: String,
    #[serde(rename = "maxFiles")] pub max_files: Option<usize>,
    #[serde(rename = "symbolLimit")] pub symbol_limit: Option<usize>,
}

pub async fn ra_build_workspace(state: &AppState, args: RaBuildWorkspaceArgs) -> Result<String, StatusCode> {
    let mut files: Vec<PathBuf> = Vec::new();
    for entry in walkdir::WalkDir::new(&args.root).into_iter().flatten(){
        let p=entry.path(); if p.is_file(){ if let Some(ext)=p.extension().and_then(|s|s.to_str()){ if ext=="rs"{ files.push(p.to_path_buf()); } } }
    }
    if let Some(m)=args.max_files { files.truncate(m); }
    let mut imported = 0usize;
    for f in files {
        let path = f.to_string_lossy().to_string();
        let txt = ra_document_symbols_to_entities(state, RaDocSymbolsToEntitiesArgs{ root: args.root.clone(), path: path.clone(), limit: args.symbol_limit }).await?;
        if let Some(n) = txt.split_whitespace().find_map(|w| w.trim_start_matches('~').parse::<usize>().ok()) { imported += n; }
    }
    let _ = state.graphiti.add_memory(AddMemoryRequest{ content: format!("RA Workspace Build: imported ~{} symbols", imported), name: Some("RA Workspace Build".into()), source: Some("serena_kg".into()), memory_type: Some("audit".into()), metadata: None, group_id: None, timestamp: None }).await;
    Ok(format!("Workspace build: imported ~{} symbols", imported))
}

#[derive(Deserialize)]
pub struct RaFileSymbolsConnectArgs { pub root:String, pub path:String, pub limit: Option<usize>, #[serde(rename = "dryRun")] pub dry_run: Option<bool> }

pub async fn ra_file_symbols_connect(state:&AppState, args: RaFileSymbolsConnectArgs) -> Result<String, StatusCode> {
    ...
    let all_nodes = state.storage.get_all_nodes().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    for n in all_nodes {
        ...
        if let Some(fp)=p.get("file_path").and_then(|v|v.as_str()){
            let is_ce=p.get("code_entity").and_then(|v|v.as_bool()).unwrap_or(false);
            if is_ce{
                let name=p.get("name").and_then(|v|v.as_str()).unwrap_or("").to_string();
                let lr=p.get("line_range").and_then(|v|v.as_array()).and_then(|arr| if arr.len()==2{ Some((arr[0].as_u64().unwrap_or(1) as u32, arr[1].as_u64().unwrap_or(1) as u32)) } else { None });
                if let Some(r)=lr{ symbol_index.insert((fp.to_string(),name,r), *n.id()); }
            } else {
                if p.get("node_type").and_then(|v|v.as_str())==Some("file") { file_index.insert(fp.to_string(), *n.id()); }
            }
        }
    }
    ...
    let file_id = *file_index.get(&args.path).unwrap_or(&{
        let id=Uuid::new_v4();
        let node=EntityNode{ id, name: args.path.clone(), entity_type: CodeEntityType::Module.to_string(), labels: vec!["File".into()], properties: serde_json::json!({"file_path": args.path, "node_type":"file"}), temporal: temporal.clone(), embedding: None };
    ...
    if let Ok(edges) = state.storage.get_edges(&file_id, Direction::Outgoing).await {
        for e in edges { if e.relationship == "CONTAINS" { existing_targets.insert(e.target_id); } }
    }
    for (name,(s,e)) in syms {
        let sym_id_opt = symbol_index.get(&(args.path.clone(), name.clone(), (s,e))).cloned();
        ...
        if let Some(sym_id) = sym_id_opt {
            if seen.insert((file_id, sym_id)){
                if existing_targets.contains(&sym_id) || edge_exists(state, file_id, sym_id, "CONTAINS").await.unwrap_or(false) {
                    counter!("graph_edges_skipped_total", "reason" => "duplicate", "relation" => "CONTAINS").increment(1);
                } else if args.dry_run.unwrap_or(false) {
                    connected += 1;
                } else {
                    let edge = Edge{ id: Uuid::new_v4(), source_id: file_id, target_id: sym_id, relationship: "CONTAINS".into(), properties: serde_json::json!({"source":"rust-analyzer"}), temporal: temporal.clone(), weight: 1.0 };
                    state.storage.create_edge(&edge).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            ...
            if args.dry_run.unwrap_or(false) {
                ...
                let id = Uuid::new_v4();
                let node=EntityNode{ id, name: name.clone(), entity_type: CodeEntityType::Function.to_string(), labels: vec!["CodeEntity".into()], properties: serde_json::json!({"code_entity":true,"name":name,"entity_type":"Function","file_path":args.path,"line_range":[s,e]}), temporal: temporal.clone(), embedding: None };
                ...
                let edge = Edge{ id: Uuid::new_v4(), source_id: file_id, target_id: id, relationship: "CONTAINS".into(), properties: serde_json::json!({"source":"rust-analyzer"}), temporal: temporal.clone(), weight: 1.0 };
                state.storage.create_edge(&edge).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
                counter!("graph_edges_created_total", "relation" => "CONTAINS").increment(1);
...
pub async fn ra_build_references_workspace(state:&AppState, args: RaBuildReferencesWorkspaceArgs) -> Result<String, StatusCode> {
    ...
    for f in files {
        ...
        tasks.push(tokio::spawn(async move {
            let _p = permit;
            let msg = ra_build_references_for_file(&s, RaBuildReferencesForFileArgs{ root, path, max_symbols: sym_limit, relation, dry_run: args.dry_run }).await;
            match msg {
                Ok(txt) => txt.split_whitespace().find_map(|w| w.parse::<usize>().ok()).unwrap_or(0),
                Err(_) => 0,
            }
        }));
    }
    let mut total_created = 0usize;
    for t in tasks { if let Ok(n) = t.await { total_created += n; } }
    Ok(format!("created ~{} reference edges across workspace", total_created))
}
...
pub async fn ra_workspace_file_symbols_connect(state:&AppState, args: RaWorkspaceFileSymbolsConnectArgs) -> Result<String, StatusCode> {
    let mut files: Vec<PathBuf> = Vec::new();
    for entry in walkdir::WalkDir::new(&args.root).into_iter().flatten(){
        let p=entry.path(); if p.is_file(){ if let Some(ext)=p.extension().and_then(|s|s.to_str()){ if ext=="rs"{ files.push(p.to_path_buf()); } } }
    }
    ...
    for f in files {
        ...
        tasks.push(tokio::spawn(async move {
            let _p = permit;
            let msg = ra_file_symbols_connect(&s, RaFileSymbolsConnectArgs{ root, path, limit: sym_limit, dry_run: args.dry_run }).await;
            match msg { Ok(txt) => txt.split_whitespace().find_map(|w| w.parse::<usize>().ok()).unwrap_or(0), Err(_) => 0 }
        }));
    }
    let mut total_connected = 0usize;
    for t in tasks { if let Ok(n) = t.await { total_connected += n; } }
    Ok(format!("connected ~{} CONTAINS edges across workspace", total_connected))
}

#[derive(Deserialize)]
pub struct RaImportAndConnectFileArgs { pub root:String, pub path:String, pub limit: Option<usize>, #[serde(rename = "dryRun")] pub dry_run: Option<bool> }

pub async fn ra_import_and_connect_file(state:&AppState, args: RaImportAndConnectFileArgs) -> Result<String, StatusCode> {
    if !args.dry_run.unwrap_or(false) {
        let _ = ra_document_symbols_to_entities(state, RaDocSymbolsToEntitiesArgs{ root: args.root.clone(), path: args.path.clone(), limit: args.limit }).await?;
    }
    let msg = ra_file_symbols_connect(state, RaFileSymbolsConnectArgs{ root: args.root, path: args.path, limit: args.limit, dry_run: args.dry_run }).await?;
    Ok(msg)
}
...
pub async fn ra_build_symbols_and_connect_workspace(state:&AppState, args: RaBuildSymbolsAndConnectWorkspaceArgs) -> Result<String, StatusCode> {
    let mut files: Vec<PathBuf> = Vec::new();
    for entry in walkdir::WalkDir::new(&args.root).into_iter().flatten(){
        let p=entry.path(); if p.is_file(){ if let Some(ext)=p.extension().and_then(|s|s.to_str()){ if ext=="rs"{ files.push(p.to_path_buf()); } } }
    }
    ...
    for f in files {
        ...
        tasks.push(tokio::spawn(async move {
            let _p = permit;
            if !args.dry_run.unwrap_or(false) {
                let _ = ra_document_symbols_to_entities(&s, RaDocSymbolsToEntitiesArgs{ root: root.clone(), path: path.clone(), limit: sym_limit }).await;
            }
            let msg = ra_file_symbols_connect(&s, RaFileSymbolsConnectArgs{ root, path, limit: sym_limit, dry_run: args.dry_run }).await;
            match msg { Ok(txt) => txt.split_whitespace().find_map(|w| w.parse::<usize>().ok()).unwrap_or(0), Err(_) => 0 }
        }));
    }
    let mut connected = 0usize; let mut imported = 0usize;
    for t in tasks { if let Ok(n) = t.await { connected += n; imported += 1; } }
    Ok(format!("workspace: imported {} files; connected ~{} CONTAINS edges", imported, connected))
}

#[derive(Deserialize)]
pub struct RaReferencesToEdgesArgs { pub root:String, pub path:String, pub line: u32, pub character:u32, pub relation: Option<String>, #[serde(rename = "dryRun")] pub dry_run: Option<bool> }

pub async fn ra_references_to_edges(state:&AppState, args: RaReferencesToEdgesArgs) -> Result<String, StatusCode> {
    // Fetch references
    let r = ra_references(RaReferencesArgs{ root: args.root.clone(), path: args.path.clone(), line: args.line as usize, character: args.character as usize }).await?;
    let refs = r.get("result").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    let relation = args.relation.clone().unwrap_or_else(|| "REFERENCES".to_string());

    // Source symbol at position
    let src_sym = match crate::serena_ra::symbol_at_position(Path::new(&args.root), Path::new(&args.path), args.line, args.character).await {
    ...
    let src_id = if let Some((name, range)) = src_sym.clone() {
        let lr=(range["start"]["line"].as_u64().unwrap_or(0) as u32 +1, range["end"]["line"].as_u64().unwrap_or(0) as u32 +1);
        if let Some(id)=symbol_index.get(&(args.path.clone(), name.clone(), lr)) { *id } else {
            let id = Uuid::new_v4();
            let node = EntityNode{ id, name: name.clone(), entity_type: CodeEntityType::Function.to_string(), labels: vec!["CodeEntity".into()], properties: serde_json::json!({"code_entity":true,"name":name,"entity_type":"Function","file_path":args.path,"line_range":[lr.0,lr.1]}), temporal: temporal.clone(), embedding: None };
            state.storage.create_node(&node).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            counter!("graph_nodes_created_total", "node_type" => "CodeEntity").increment(1);
            id
        }
    ...
    let mut seen = std::collections::HashSet::new();
    for loc in refs {
        let uri = match loc.get("uri").and_then(|v| v.as_str()) { Some(u) => u, None => continue };
        let range = match loc.get("range") { Some(r) => r.clone(), None => serde_json::json!({"start":{"line":0,"character":0}}) };
        let pl = range.get("start").and_then(|s| s.get("line")).and_then(|x| x.as_u64()).unwrap_or(0) as u32;
        let pc = range.get("start").and_then(|s| s.get("character")).and_then(|x| x.as_u64()).unwrap_or(0) as u32;
        let ref_path = match url::Url::parse(uri).ok().and_then(|u| u.to_file_path().ok()) { Some(p)=> p.to_string_lossy().to_string(), None => continue };

        // referencing symbol at reference location
        let ref_sym = match crate::serena_ra::symbol_at_position(Path::new(&args.root), Path::new(&ref_path), pl, pc).await { Ok(v)=>v, Err(_)=>None };
        let ref_id = if let Some((name, _kind, rng)) = ref_sym {
            let lr=(rng["start"]["line"].as_u64().unwrap_or(0) as u32 +1, rng["end"]["line"].as_u64().unwrap_or(0) as u32 +1);
            if let Some(id)=symbol_index.get(&(ref_path.clone(), name.clone(), lr)) { *id } else {
                let id = Uuid::new_v4();
                let node = EntityNode{ id, name: name.clone(), entity_type: CodeEntityType::Function.to_string(), labels: vec!["CodeEntity".into()], properties: serde_json::json!({"code_entity":true,"name":name,"entity_type":"Function","file_path":ref_path,"line_range":[lr.0,lr.1]}), temporal: temporal.clone(), embedding: None };
                state.storage.create_node(&node).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
                id
            }
        } else {
            *file_index.get(&ref_path).unwrap_or(&{
                let id=Uuid::new_v4();
                let node=EntityNode{ id, name: ref_path.clone(), entity_type: CodeEntityType::Module.to_string(), labels: vec!["File".into()], properties: serde_json::json!({"file_path": ref_path, "node_type":"file"}), temporal: temporal.clone(), embedding: None };
                let _=state.storage.create_node(&node);
                counter!("graph_nodes_created_total", "node_type" => "File").increment(1);
                id
            })
        ...
        if edge_exists(state, ref_id, src_id, &relation).await? {
            ...
            let edge = Edge{ id: Uuid::new_v4(), source_id: ref_id, target_id: src_id, relationship: relation.clone(), properties: serde_json::json!({"source":"rust-analyzer"}), temporal: temporal.clone(), weight: 1.0 };
            state.storage.create_edge(&edge).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
#[derive(Deserialize)]
pub struct RaSymbolDefinitionEdgeArgs { pub root:String, pub path:String, pub line:u32, pub character:u32, pub relation: Option<String> }

pub async fn ra_symbol_definition_edge(state:&AppState, args: RaSymbolDefinitionEdgeArgs) -> Result<String, StatusCode> {
    ...
    let def = ra_definition(RaDefinitionArgs{ root: args.root.clone(), path: args.path.clone(), line: args.line as usize, character: args.character as usize }).await?;
    let (def_uri, def_range) = if let Some(obj)=def.as_object(){
        if let Some(arr)=obj.get("result").and_then(|v| v.as_array()).or_else(|| obj.get("location").and_then(|v| v.as_array())){
            if let Some(first)=arr.get(0){
                ...
                (uri, range)
            } else { (String::new(), serde_json::json!({})) }
        } else { (String::new(), serde_json::json!({})) }
    } else { (String::new(), serde_json::json!({})) };
    if def_uri.is_empty(){ return Ok("no definition".into()); }
    let dst_symbol = crate::serena_ra::find_target_symbol_name(Path::new(&args.root), &def_uri, &def_range).await;
    ...
    let dst_id = if let Some(name)=dst_symbol {
        ...
        let lr=(def_range["start"]["line"].as_u64().unwrap_or(0) as u32 +1, def_range["end"]["line"].as_u64().unwrap_or(0) as u32 +1);
        if let Some(id)=symbol_index.get(&(dst_path.clone(), name.clone(), lr)) { *id } else {
            let id = Uuid::new_v4();
            let node=EntityNode{ id, name: name.clone(), entity_type: CodeEntityType::Function.to_string(), labels: vec!["CodeEntity".into()], properties: serde_json::json!({"code_entity":true,"name":name,"entity_type":"Function","file_path":dst_path,"line_range":[lr.0,lr.1]}), temporal: temporal.clone(), embedding: None };
            state.storage.create_node(&node).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            id
        }
    } else {
        *file_index.get(&dst_path).unwrap_or(&{
            let id=Uuid::new_v4();
            let node=EntityNode{ id, name: dst_path.clone(), entity_type: CodeEntityType::Module.to_string(), labels: vec!["File".into()], properties: serde_json::json!({"file_path": dst_path, "node_type":"file"}), temporal: temporal.clone(), embedding: None };
            let _=state.storage.create_node(&node);
            id
        })
    };

    let relation = args.relation.clone().unwrap_or_else(|| "DEFINES".to_string());
    if edge_exists(state, src_id, dst_id, &relation).await? {
        counter!("graph_edges_skipped_total", "reason" => "duplicate", "relation" => relation.clone()).increment(1);
        return Ok("exists".into());
    }
    let edge = Edge{ id: Uuid::new_v4(), source_id: src_id, target_id: dst_id, relationship: relation.clone(), properties: serde_json::json!({"source":"rust-analyzer"}), temporal: temporal.clone(), weight: 1.0 };
    state.storage.create_edge(&edge).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub struct RaBuildReferencesForFileArgs {
    pub root: String,
    pub path: String,
    #[serde(rename = "maxSymbols")] pub max_symbols: Option<usize>,
    pub relation: Option<String>,
    #[serde(rename = "dryRun")] pub dry_run: Option<bool>,
}

pub async fn ra_build_references_for_file(state:&AppState, args: RaBuildReferencesForFileArgs) -> Result<String, StatusCode> {
    let res = ra_document_symbols(RaDocumentSymbolsArgs{ root: args.root.clone(), path: args.path.clone() }).await?;
    fn collect_positions(v:&serde_json::Value, out:&mut Vec<(String,u32,u32)>) {
        if let Some(arr)=v.as_array(){ for it in arr { collect_positions(it,out);} return; }
        if let Some(o)=v.as_object(){
            if let Some(n)=o.get("name").and_then(|s| s.as_str()){
                let sline = o.get("range").and_then(|r| r.get("start")).and_then(|s| s.get("line")).and_then(|x| x.as_u64()).unwrap_or(0) as u32;
                let schar = o.get("range").and_then(|r| r.get("start")).and_then(|s| s.get("character")).and_then(|x| x.as_u64()).unwrap_or(0) as u32;
                out.push((n.to_string(), sline, schar));
            }
            if let Some(ch)=o.get("children"){ collect_positions(ch,out); }
        }
    }
    let mut syms = Vec::new(); collect_positions(&res,&mut syms);
    if let Some(m)=args.max_symbols { syms.truncate(m); }
    let mut total_created = 0usize;
    for (_name, sl, sc) in syms {
        let msg = ra_references_to_edges(state, RaReferencesToEdgesArgs{ root: args.root.clone(), path: args.path.clone(), line: sl, character: sc, relation: args.relation.clone(), dry_run: args.dry_run }).await?;
        if let Some(n) = msg.split_whitespace().find_map(|w| w.parse::<usize>().ok()) { total_created += n; }
    }
    Ok(format!("created ~{} reference edges", total_created))
}

#[derive(Deserialize)]
pub struct OnboardingDoneArgs { pub project: Option<String> }
