use crate::vectordb::{VectorDatabase, VectorHit, VectorInsert};
use anyhow::Result;
use qdrant_client::qdrant::value::Kind as Vk;
use qdrant_client::qdrant::{Condition, DeletePointsBuilder, Filter};
use qdrant_client::qdrant::{
    CreateCollectionBuilder, SearchParams, UpsertPointsBuilder, VectorParamsBuilder,
};
...
impl QdrantVectorDb {
    ...
    async fn search_rest_inner(
        ...
        if !resp.status().is_success() {
            anyhow::bail!("REST search HTTP {}", resp.status());
        }
        ...
        let out = sr
            ...
            .map(|p| {
                ...
                let int_of =
                    |k: &str| -> i64 { payload.get(k).and_then(|v| v.as_i64()).unwrap_or(0) };
                VectorHit {
                    score: p.score,
...
impl VectorDatabase for QdrantVectorDb {
    fn ensure_collection(&self, name: &str, dimension: usize) -> Result<()> {
        run_async_blocking(async {
            match self.client.collection_exists(name).await {
                Ok(exists) => {
                    if !exists {
                        let req = CreateCollectionBuilder::new(name).vectors_config(
                        ...
                        if let Err(e) = self.client.create_collection(req).await {
                            ...
                            let body = serde_json::json!({ "vectors": { "size": dimension as u64, "distance": "Cosine" } });
                            let client = reqwest::Client::new();
                ...
                Err(e) => {
                    eprintln!(
                        "[warn] gRPC collection_exists failed: {} — checking via REST",
                        e
                    );
                    let url = format!(
                        "{}/collections/{}",
    ...
    fn has_collection(&self, name: &str) -> Result<bool> {
        run_async_blocking(async { Ok(self.client.collection_exists(name).await.unwrap_or(false)) })
    }
    ...
    fn drop_collection(&self, name: &str) -> Result<()> {
        run_async_blocking(async {
            ...
            match self.client.delete_collection(name).await {
                ...
                Err(e) => {
                    eprintln!(
                        "[warn] gRPC delete_collection failed: {} — falling back to REST",
                        e
                    );
                    let url = format!(
                        "{}/collections/{}",
                    ...
                    if !resp.status().is_success() {
                        anyhow::bail!("REST delete_collection HTTP {}", resp.status());
                    }
                    Ok(())
                }
            }
    ...
    fn insert(&self, name: &str, points: &[VectorInsert]) -> Result<()> {
        ...
        let pts: Vec<PointStruct> = points
            ...
            .map(|p| {
                ...
                payload.insert(
                    "content".to_string(),
                    Value {
                        kind: Some(Vk::StringValue(p.content.clone())),
                    },
                );
                payload.insert(
                    "relative_path".to_string(),
                    Value {
                        kind: Some(Vk::StringValue(p.relative_path.clone())),
                    },
                );
                payload.insert(
                    "start_line".to_string(),
                    Value {
                        kind: Some(Vk::IntegerValue(p.start_line as i64)),
                    },
                );
                payload.insert(
                    "end_line".to_string(),
                    Value {
                        kind: Some(Vk::IntegerValue(p.end_line as i64)),
                    },
                );
                payload.insert(
                    "language".to_string(),
                    Value {
                        kind: Some(Vk::StringValue(p.language.clone())),
                    },
                );
                payload.insert(
                    "file_extension".to_string(),
                    Value {
                        kind: Some(Vk::StringValue(p.file_extension.clone())),
                    },
                );
        ...
        run_async_blocking(async move {
            ...
            match self.client.upsert_points(req).await {
                ...
                Err(e) => {
                    eprintln!(
                        "[warn] gRPC upsert failed: {} — falling back to REST /points/upsert",
                        e
                    );
                    ...
                    for p in points.iter() {
                        ...
                        rest_points.push(serde_json::json!({ "id": id_num, "vector": p.vector, "payload": payload }));
                    }
                    ...
                    let resp = client.post(url).json(&body).send().await?;
                    if !resp.status().is_success() {
                        anyhow::bail!("REST upsert HTTP {}", resp.status());
                    }
                    Ok(())
                }
            }
    ...
    fn search(&self, name: &str, vector: &[f32], top_k: usize) -> Result<Vec<VectorHit>> {
        run_async_blocking(async move {
            ...
            let result = match self.client.query(req).await {
                ...
                Err(e) => {
                    eprintln!(
                        "[warn] gRPC query failed: {} — falling back to REST /points/search",
                        e
                    );
                    ...
                    let str_of = |k: &str| -> String {
                        payload
                            ...
                            .and_then(|v| match &v.kind {
                                Some(Vk::StringValue(s)) => Some(s.clone()),
                                _ => None,
                            })
                            .unwrap_or_default()
                    };
    ...
    fn delete_by_relative_paths(&self, name: &str, rel_paths: &[String]) -> Result<()> {
        ...
        run_async_blocking(async move {
            let conditions: Vec<qdrant_client::qdrant::Condition> = rel_paths
                .iter()
                .map(|p| Condition::matches("relative_path", p.clone()))
                .collect();
            ...
            let req = DeletePointsBuilder::new(name)
                .points(filter.clone())
                .wait(true);
            match self.client.delete_points(req).await {
                ...
                Err(e) => {
                    eprintln!(
                        "[warn] gRPC delete failed: {} — falling back to REST /points/delete",
                        e
                    );
                    let url = format!(
                    ...
                    let conditions_json: Vec<serde_json::Value> = rel_paths
                        .iter()
                        .map(|p| serde_json::json!({"key": "relative_path","match": {"value": p}}))
                        .collect();
                    let filter_json = if conditions_json.len() == 1 {
                        serde_json::json!({"must": conditions_json})
                    } else {
                        serde_json::json!({"should": conditions_json})
                    };
                    let body = serde_json::json!({ "filter": filter_json });
                    let client = reqwest::Client::new();
                    let resp = client.post(url).json(&body).send().await?;
                    if !resp.status().is_success() {
                        anyhow::bail!("REST delete HTTP {}", resp.status());
                    }
                    Ok(())
                }
            }
