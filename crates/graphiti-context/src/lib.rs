...
impl Default for ContextConfig {
    fn default() -> Self {
        Self {
            ...
            use_candle_gemma: std::env::var("USE_CANDLE_GEMMA").map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false),
...
impl Context {
    pub fn new(cfg: Option<ContextConfig>) -> Result<Self> {
        ...
        #[allow(unused_mut)]
        let mut embedder: Box<dyn EmbeddingModel> = Box::new(HashingEmbedding::new(cfg.fallback_dim));

        // Prefer Qwen3-Embedding (1024-dim) via embed_anything if requested
        if std::env::var("USE_QWEN3").map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false) {
        ...
        if cfg.use_candle_gemma {
            ...
            if std::env::var("HF_HUB_TOKEN").is_err() {
                if let Ok(tok) = std::env::var("HF_TOKEN").or_else(|_| std::env::var("HUGGINGFACE_TOKEN")) {
                    std::env::set_var("HF_HUB_TOKEN", tok);
                }
            }
            let model_dir = cfg.embedding_model_dir.as_ref().map(|s| std::path::PathBuf::from(s));
        ...
        let vectordb: Option<Box<dyn VectorDatabase>> = if cfg.use_qdrant {
            let url = cfg.qdrant_url.clone().unwrap_or_else(|| "http://127.0.0.1:6334".to_string());
            let db = crate::vectordb_qdrant::QdrantVectorDb::new(&url)?;
            Some(Box::new(db))
        } else { None };

        Ok(Self { cfg, rules, chunker, storage, embedder, vectordb })
    }

    pub fn collection_name(&self, codebase_path: &Path) -> String {
        let normalized = codebase_path.canonicalize().unwrap_or_else(|_| codebase_path.to_path_buf());
        let mut hasher = Md5::new();
        hasher.update(normalized.to_string_lossy().as_bytes());
        let digest = hasher.finalize();
        let hash = format!("{:x}", digest);
        let prefix = if self.cfg.hybrid_mode { "hybrid_code_chunks" } else { "code_chunks" };
    ...
    pub fn index_codebase<F>(&self, codebase_path: &Path, mut progress: Option<F>, force_reindex: bool) -> Result<(usize, usize)>
    where
        F: FnMut(IndexProgress),
    {
        ...
        if let Some(cb) = progress.as_mut() {
            cb(IndexProgress { phase: ProgressPhase::Preparing, current: 0, total: 100, percentage: 0 });
        }

        let files = scan_code_files(codebase_path, &self.rules)?;
        if let Some(cb) = progress.as_mut() {
            cb(IndexProgress { phase: ProgressPhase::Scanning, current: 5, total: 100, percentage: 5 });
        }
        ...
        for (i, file) in files.iter().enumerate() {
            ...
            if let Some(cb) = progress.as_mut() {
                ...
                cb(IndexProgress { phase: ProgressPhase::Indexing, current: i + 1, total: total_files, percentage: pct.min(100) });
            }
        }

        if let Some(cb) = progress.as_mut() {
            cb(IndexProgress { phase: ProgressPhase::Completed, current: processed_files, total: total_files, percentage: 100 });
        }
        Ok((processed_files, total_chunks))
    }

    pub fn search(&self, codebase_path: &Path, query: &str, top_k: usize) -> Result<Vec<SearchResult>> {
        let name = self.collection_name(codebase_path);
        if let Some(db) = &self.vectordb {
            ...
            let out = hits
                .into_iter()
                .map(|h| SearchResult { content: h.content, relative_path: h.relative_path, start_line: h.start_line, end_line: h.end_line, language: h.language, score: h.score })
                .collect();
            Ok(out)
        } else {
    ...
    pub fn get_embedder_name(&self) -> &'static str { self.embedder.name() }
    pub fn get_embedder_dim(&self) -> usize { self.embedder.dimension() }
    pub fn is_qdrant_enabled(&self) -> bool { self.vectordb.is_some() }
